...
const knowledgeType = (text, scope) => {
  ...
  const declared = field(frontmatter, 'knowledgeType') || field(frontmatter, 'knowledge_type') || field(frontmatter, 'knowledge-type')
  if (knowledgeTypes.has(declared)) return declared
  if (scope === 'project' || scope === 'decision') return 'fact'
  if (scope === 'synthesis') return 'hypothesis'
  return 'wiki-record'
}
...
const personalSource = (value, memberId) => new RegExp(`^members/${memberId}/(WIKI_SCHEMA\\.md|wiki/.*\\.md)$`).test(value || '') && !/\.private\.md$|\/(raw|output|\.obsidian|node_modules)\//.test(value || '')
...
const validSnapshot = value => {
  ...
  const sourcesOkay = value.documents.every(item => item.id && item.title && knowledgeTypes.has(item.knowledgeType) && /^[a-f0-9]{64}$/.test(item.contentDigest || '') && (item.scope === 'personal' ? safeId.test(item.memberId || '') && personalSource(item.source, item.memberId) : sharedSource(item.source)))
  ...
  return value.members.length > 0 && personal.length > 0 && value.members.every(member => safeId.test(member.id || '') && member.documentCount > 0 && Array.isArray(member.documentIds) && member.documentCount === member.documentIds.length && member.documentIds.every(id => personalById.get(id)?.memberId === member.id)) && JSON.stringify(value.manifest) === JSON.stringify(manifestFor(value.documents))
}
const validEvidence = (value, snapshot) => {
  if (!value || value.version < 3 || !Array.isArray(value.documents) || !value.documents.length) return false
  const sourcesOkay = value.documents.every(item => item.id && typeof item.body === 'string' && knowledgeTypes.has(item.knowledgeType) && item.contentDigest === digest(item.body) && (item.scope === 'personal' ? safeId.test(item.memberId || '') && personalSource(item.source, item.memberId) : sharedSource(item.source)))
  const snapshotIds = new Set(snapshot.documents.map(item => item.id)); const evidenceIds = new Set(value.documents.map(item => item.id))
  return sourcesOkay && snapshotIds.size === evidenceIds.size && [...snapshotIds].every(id => evidenceIds.has(id)) && JSON.stringify(value.manifest) === JSON.stringify(manifestFor(value.documents)) && JSON.stringify(snapshot.manifest) === JSON.stringify(value.manifest)
}
...
async function member(memberId) { const root = join(membersRoot, memberId); const wiki = join(root, 'wiki'); const files = []; for (const file of [join(root, 'WIKI_SCHEMA.md'), join(wiki, 'index.md')]) try { files.push(await load(file)) } catch {} try { for (const file of await walk(wiki)) files.push(await load(file)) } catch {} const unique = [...new Map(files.filter(Boolean).map(file => [file.absolute, file])).values()]; const documents = unique.map(file => ({ id: documentId(memberId, file.absolute), memberId, author: memberId, category: '개인 Wiki', scope: 'personal', source: source(file.absolute), title: title(file.text, memberId), updated: field(file.text, 'updated') || field(file.text, 'created') || 'snapshot', status: field(file.text, 'status') || 'unclassified', knowledgeType: knowledgeType(file.text, 'personal'), contentDigest: digest(file.text), excerpt: excerpt(file.text) })); const evidence = unique.map(file => ({ id: documentId(memberId, file.absolute), memberId, scope: 'personal', source: source(file.absolute), title: title(file.text, memberId), knowledgeType: knowledgeType(file.text, 'personal'), contentDigest: digest(file.text), body: file.text })); return { id: memberId, memberId, displayName: memberId, documentIds: documents.map(document => document.id), documentCount: documents.length, role: inferGuildRole(documents), activity: await publicActivity(memberId), documents, evidence } }
...
const sharedDocuments = []; const sharedEvidence = []; for (const item of shared) try { const file = await load(join(repoRoot, item.path)); if (file) { const document = { ...item, source: source(file.absolute), title: title(file.text, item.id), updated: field(file.text, 'updated') || 'snapshot', status: field(file.text, 'status') || (item.scope === 'decision' ? 'no-recorded-decision' : 'unclassified'), knowledgeType: knowledgeType(file.text, item.scope), contentDigest: digest(file.text), excerpt: excerpt(file.text) }; sharedDocuments.push(document); sharedEvidence.push({ id: document.id, scope: document.scope, source: document.source, title: document.title, knowledgeType: document.knowledgeType, contentDigest: document.contentDigest, body: file.text }) } } catch {}
...
const snapshot = { version: 3, generatedAt: `${latestUpdated}T00:00:00.000Z`, sourceState: indexed.length ? 'indexed' : 'verified-empty', boundary: 'WIKI SNAPSHOT · server-side LLM grounding when configured · explicit deterministic demo fallback · no browser filesystem access', decisionState: 'Official decisions are represented only by decisions/ documents.', manifest, projectContext, members: indexed.map(({ documents: memberDocuments, evidence, ...item }) => item), documents }
const evidenceDocuments = [...indexed.flatMap(item => item.evidence), ...sharedEvidence]
await mkdir(dirname(output), { recursive: true }); await mkdir(dirname(evidenceOutput), { recursive: true }); await writeFile(output, `${JSON.stringify(snapshot, null, 2)}\n`); await writeFile(evidenceOutput, `${JSON.stringify({ version: 3, manifest, documents: evidenceDocuments }, null, 2)}\n`)
