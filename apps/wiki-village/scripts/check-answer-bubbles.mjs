...
if (!answerBubbleView({ ...resolved, reply: { ...reply, answer: '가'.repeat(121) } })?.text.endsWith('…')) throw new Error('Answer bubble did not mark a truncated excerpt')
...
if (!isValidatedAnswerEnvelope(reply, atlas) || isValidatedAnswerEnvelope({ ...reply, memberId: 'lumi' }, atlas) || isValidatedAnswerEnvelope({ ...reply, citations: 'not-an-array' }, atlas) || isValidatedAnswerEnvelope({ ...reply, mode: 'unclassified' }, atlas) || isValidatedAnswerEnvelope({ ...reply, citations: [{ path: 'members/atlas/wiki/private.md' }] }, atlas)) throw new Error('Malformed or cross-member answer envelope entered the bubble path')
