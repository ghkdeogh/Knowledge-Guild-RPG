...
const draft = localDraft('기술 학습 결과를 팀과 정리하려는 프로젝트입니다.'); const identity = { memberId: 'deploy-check', displayName: '배포 확인', workingContext: '읽기 전용 blueprint preview를 확인합니다.' }; const previewResponse = await invoke(onboarding, 'POST', '/api/onboarding', JSON.stringify({ action: 'preview', blueprint: draft.blueprint, identity })); const previewBody = JSON.parse(previewResponse.payload); if (previewResponse.status !== 200 || !previewBody.preview?.digest || !previewBody.preview.files.every(file => file.path.startsWith('members/deploy-check/'))) throw new Error('Vercel onboarding did not allow personal-only blueprint preview')
const saveResponse = await invoke(onboarding, 'POST', '/api/onboarding', JSON.stringify({ action: 'save', blueprint: draft.blueprint, identity, expectedDigest: previewBody.preview.digest })); if (saveResponse.status !== 400 || JSON.parse(saveResponse.payload).code !== 'invalid-action') throw new Error('Vercel onboarding exposed a persistent write action')
