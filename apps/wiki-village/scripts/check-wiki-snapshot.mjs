...
const builder = await readFile(join(appRoot, 'scripts', 'build-wiki-snapshot.mjs'), 'utf8'); if (!builder.includes("'private'") || !builder.includes("'secrets'")) throw new Error('Private/secrets source exclusions are missing'); if (builder.includes("[join(root, 'CONTEXT.md'), join(root, 'WIKI_SCHEMA.md'), ...(await approvedWikiFiles")) throw new Error('Member context/schema leaked into the public snapshot source list')
