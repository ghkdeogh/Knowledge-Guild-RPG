...
const paths = text => [...new Map(String(text || '').split(/\r?\n/).map(line => classifyWikiPath(line.replace(/^..\s+/, '').trim())).filter(Boolean).map(item => [item.path, item])).values()]
...
export async function repositoryStatus({ repoRoot, fetch = false } = {}) {
  ...
  try {
    ...
    const [branch, head, upstreamResult, dirtyText] = await Promise.all([git(repoRoot, ['branch', '--show-current']), git(repoRoot, ['rev-parse', 'HEAD']), git(repoRoot, ['rev-parse', '--abbrev-ref', '--symbolic-full-name', '@{upstream}']).catch(() => ''), git(repoRoot, ['status', '--porcelain', '--untracked-files=all'])])
    ...
    const dirty = paths(dirtyText); const tracked = paths(await git(repoRoot, ['ls-files'])); const memberIds = [...new Set([...tracked, ...dirty, ...remoteNews].filter(item => item.memberId).map(item => item.memberId))]; const members = {}
    for (const id of memberIds) { const pathsForMember = [`members/${id}/wiki`, `members/${id}/harnesses`, `members/${id}/WIKI_INDEX.md`, `members/${id}/ACTIVITY_LOG.md`]; const last = await commitFor(repoRoot, 'HEAD', pathsForMember); const remote = upstream ? await commitFor(repoRoot, upstream, pathsForMember) : { commit: null, date: null }; members[id] = { dirty: dirty.some(value => value.memberId === id), remoteNews: remoteNews.some(value => value.memberId === id), remoteTip: remote.commit, lastCommit: last.commit, lastDate: last.date } }
    const projectLast = await commitFor(repoRoot, 'HEAD', ['projects/wiki', 'projects/harnesses', 'projects/WIKI_INDEX.md', 'projects/ACTIVITY_LOG.md'])
