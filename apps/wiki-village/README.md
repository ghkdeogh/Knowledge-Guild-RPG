...
Deploy from the repository root: `npx vercel --yes`. A temporary deployment can be claimed in Vercel, then configure the server-only project variables `OPENAI_API_KEY`, `OPENAI_MODEL=gpt-5.6-terra`, and `OPENAI_REASONING_EFFORT=low`, followed by `npx vercel --prod`. The root `vercel.json` builds this app into `apps/wiki-village/dist` and serves the same-origin `POST /api/wiki-chat` function. Personal evidence is limited to allowlisted `members/*/wiki/**/*.md`; `CONTEXT.md`, `WIKI_SCHEMA.md`, raw, output, private, hidden, Obsidian, template, and clipper files are excluded. Approved project records are represented separately. Do not mark a deployment production-ready until the deployed endpoint is verified.
