...
async function readInputs(repoRoot, memberId) {
  ...
  const [profile, context, principles] = await Promise.all([safeFile(profilePath), safeFile(contextPath), readFile(principlesPath, 'utf8')])
  if (!principles.trim()) fail('LLM Wiki 운영 원칙을 읽을 수 없습니다.', 'prompt-missing')
  if (!context) return { scope, profile, principles, inputMissing: true, transitionalClaudeOnly: await exists(claudePath), profileDigest: contentDigest(profile), principlesDigest: contentDigest(principles) }
  const claude = await safeFile(claudePath)
  ...
  return { scope, profile, claude, context, principles, profileDigest: contentDigest(profile), contextDigest: contentDigest(context), principlesDigest: contentDigest(principles), existingClaudeDigest: claude ? contentDigest(claude) : null }
}
function evidenceSections(profile, personal, personalDocument = 'CONTEXT') {
...
function offlinePlan(profile, personal, personalDocument = 'CONTEXT') {
...
function operationsBlock(plan) { return `${managedStart}\n\n## Personal LLM Wiki operations\n\n- raw/ is immutable source of truth, organized by evidence-backed input types; read raw/CLAUDE.md before ingest.\n- When a user says "인제스트해줘", follow prompts/ingest.md: read the selected raw source without changing it, present analysis, ask its three questions one at a time, then require a preview approval before Wiki writes.\n- wiki/ is the AI-compiled knowledge layer, organized by evidence-backed knowledge areas; read wiki/CLAUDE.md before edits.\n- output/ (shown to users as Output) is the lowercase cross-platform-safe local result layer, organized by evidence-backed output types; read output/CLAUDE.md before creating results.\n- Ingest compiles one source into linked wiki pages, updates wiki/index.md, and appends wiki/log.md.\n- Query reads wiki/index.md first and writes a durable result only with approval.\n- Lint identifies contradictions, staleness, orphaned pages, missing links, and knowledge gaps.\n- Label facts, personal opinions, hypotheses, and AI inferences. Never create harnesses/, *.SKILL.md, WIKI_INDEX.md, or ACTIVITY_LOG.md in this personal Wiki.\n\nCurrent selected areas: raw(${plan.raw.map(x => x.id).join(', ')}), wiki(${plan.wiki.map(x => x.id).join(', ')}), output(${plan.output.map(x => x.id).join(', ')}).\n\n${managedEnd}\n` }
...
function freshClaudeContract(plan) { return `# Personal LLM Wiki Contract\n\nThis is a cross-agent operational contract, not a copy or summary of personal inputs. Before any personal Wiki task, read the selected member's PROFILE.md (detailed approved profile), CONTEXT.md (concise every-run context), this CLAUDE.md, and WIKI_SCHEMA.md. Follow prompts/llm-wiki.md for the Karpathy persistent-Wiki principles.\n\n## Personalized map\n\n${['raw', 'wiki', 'output'].map(layer => `- ${layer}/: ${plan[layer].map(item => `\`${item.id}/\` (${item.label}: ${item.purpose})`).join(', ')}`).join('\n')}\n\n## Operating boundaries\n\n- Treat raw/ as immutable source material; never alter, move, or reinterpret raw evidence without keeping provenance clear.\n- Let the LLM maintain wiki/ as the compiled, interlinked knowledge layer; distinguish facts, opinions, hypotheses, and inferences.\n- Keep output/ local results grounded in Wiki pages and user-approved durable outputs.\n- For every ingest, update relevant Wiki pages, wiki/index.md, and append wiki/log.md. For queries, read the index first; for lint, check contradictions, staleness, orphan pages, missing links, and gaps.\n\n${operationsBlock(plan)}` }
function folderClaude(layer, entries) { const mutability = layer === 'raw' ? 'Raw sources are immutable: read only; never edit, move, or rename them.' : layer === 'wiki' ? 'The LLM maintains linked compiled knowledge with source links and clear fact/opinion/hypothesis/inference labels.' : 'Outputs cite their Wiki basis and remain local working artifacts.'; const ingest = layer === 'wiki' ? '\n\nFor an approved ingest, update the relevant compiled pages, wiki/index.md, and wiki/log.md together; follow prompts/ingest.md for the conversation and preview boundary.' : ''; return `# ${layer}/ CLAUDE.md — Personal LLM Wiki layer contract\n\n${mutability}${ingest}\n\n## Selected areas\n\n${entries.map(entry => `- \`${entry.id}/\`: ${entry.purpose}`).join('\n')}\n\nDo not ingest CLAUDE.md or .gitkeep as source content.\n` }
function renderedFiles(memberId, plan, existingClaude = null) {
  const base = `members/${memberId}`; const files = {}
  files[`${base}/CLAUDE.md`] = existingClaude === null ? freshClaudeContract(plan) : claudeWithRules(existingClaude, plan)
...
async function migration(repoRoot, memberId, candidateFiles = [], { rootClaude = null } = {}) {
  ...
  const existingClaude = rootClaude !== null
  return { required: existingClaude || structural.length > 0 || hasManagedRules, kind: hasManagedRules ? 'managed-rules-update' : existingClaude ? 'existing-claude-update' : structural.length ? 'existing-structure' : 'fresh', actions: [...actions.values()].sort((a, b) => a.path.localeCompare(b.path)) }
}
export async function startPersonalWikiInit(input = {}, options = {}) {
  const memberId = safeMemberId(input.memberId)
  const inputs = await readInputs(options.repoRoot, memberId)
  if (inputs.inputMissing) {
    const question = inputs.transitionalClaudeOnly
      ? '이 member는 이전 전환 상태(PROFILE.md + CLAUDE.md, CONTEXT.md 없음)입니다. CLAUDE.md를 입력으로 사용하거나 자동 변환하지 않습니다. 온보딩으로 CONTEXT.md를 명시적으로 보완한 뒤 다시 초기화해 주세요.'
      : 'PROFILE.md와 CONTEXT.md가 모두 필요합니다. 온보딩을 완료한 뒤 다시 초기화해 주세요.'
    return {
      state: { version: 1, phase: 'insufficient-context', memberId, profileDigest: inputs.profileDigest, principlesDigest: inputs.principlesDigest },
      events: [event('session.started', { command: 'personal-wiki-init', memberId }), event('clarification.required', { question, count: 0, maximum: 0, ...(inputs.transitionalClaudeOnly ? { compatibility: 'profile-claude-without-context' } : {}) }), event('session.completed', { phase: 'insufficient-context' })],
      result: { status: 'context-missing', mode: 'offline-conservative', providerStatus: 'not-consented', ...(inputs.transitionalClaudeOnly ? { compatibility: 'profile-claude-without-context' } : {}) }
    }
  }
  const { profile, context, claude, principles, profileDigest, contextDigest, principlesDigest, existingClaudeDigest } = inputs; const proposed = await proposedPlan(profile, context, 'CONTEXT', principles, { ...options, providerApproved: input.providerApproved === true })
  if (!proposed.plan) { const state = { version: 1, phase: 'clarification', memberId, profile, context, existingClaude: claude, profileDigest, contextDigest, principlesDigest, existingClaudeDigest, providerApproved: input.providerApproved === true, clarificationCount: 0 }; return { state, events: [event('session.started', { command: 'personal-wiki-init', memberId }), event('provider.consent', { enabled: input.providerApproved === true }), event('clarification.required', { question: proposed.clarification, count: 1, maximum: 1 }), event('session.completed', { phase: 'insufficient-context' })], result: { status: 'insufficient-context', mode: proposed.mode, providerStatus: proposed.providerStatus } }
  }
  const candidate = renderedFiles(memberId, proposed.plan, claude); const existing = await migration(options.repoRoot, memberId, Object.entries(candidate).map(([path, content]) => ({ path, content })), { rootClaude: claude }); const state = { version: 1, phase: 'preview', memberId, context, existingClaude: claude, profileDigest, contextDigest, principlesDigest, existingClaudeDigest, plan: proposed.plan, mode: proposed.mode, providerStatus: proposed.providerStatus, migration: existing }; const preview = previewPersonalWikiInit(state)
  return { state, events: [event('session.started', { command: 'personal-wiki-init', memberId }), event('provider.consent', { enabled: input.providerApproved === true }), event('map.proposed', { map: mapSummary(proposed.plan), mode: proposed.mode }), ...(existing.required ? [event('migration.required', { actions: existing.actions })] : []), event('files.planned', { files: preview.files.map(file => file.path), requirements: preview.requirements, digest: preview.digest }), event('approval.required', { digest: preview.digest, reason: existing.required ? '기존 구조가 있어 migration 승인도 필요합니다.' : '미리보기와 digest를 확인한 뒤 명시적으로 승인해야 저장할 수 있습니다.' })], result: { preview, migration: existing, mode: proposed.mode, providerStatus: proposed.providerStatus } }
}
...
const requiredOutcomes = memberId => [{ id: 1, label: '목적 기반 raw → wiki → output 구조', paths: [`members/${memberId}/raw/`, `members/${memberId}/wiki/`, `members/${memberId}/output/`] }, { id: 2, label: 'root CLAUDE Wiki 운영 규칙', paths: [`members/${memberId}/CLAUDE.md`] }, { id: 3, label: 'layer scoped CLAUDE', paths: [`members/${memberId}/raw/CLAUDE.md`, `members/${memberId}/wiki/CLAUDE.md`, `members/${memberId}/output/CLAUDE.md`] }, { id: 4, label: 'Wiki index 초기화', paths: [`members/${memberId}/wiki/index.md`] }, { id: 5, label: 'Wiki log 초기화', paths: [`members/${memberId}/wiki/log.md`] }]
export function previewPersonalWikiInit(state) { if (!state || state.version !== 1 || state.phase !== 'preview') fail('초기화 preview 세션이 필요합니다.', 'invalid-session'); const files = renderedFiles(state.memberId, state.plan, state.existingClaude); const keep = new Set((state.migration?.actions || []).filter(item => item.action === 'keep').map(item => `members/${state.memberId}/${item.path}`)); for (const path of keep) delete files[path]; const requirements = requiredOutcomes(state.memberId); return { kind: 'personal-wiki-init', mode: state.mode, migration: state.migration, profileDigest: state.profileDigest, contextDigest: state.contextDigest, principlesDigest: state.principlesDigest, existingClaudeDigest: state.existingClaudeDigest, requirements, plan: state.plan, files: Object.entries(files).map(([path, content]) => ({ path, content })), digest: digest({ files, requirements, mode: state.mode, migration: state.migration, profileDigest: state.profileDigest, contextDigest: state.contextDigest, principlesDigest: state.principlesDigest, existingClaudeDigest: state.existingClaudeDigest }) } }
async function atomicSave(preview, repoRoot) {
  ...
  try {
    ...
    if (!(await exists(join(target, 'CONTEXT.md'))) || contentDigest(await readFile(join(target, 'CONTEXT.md'), 'utf8')) !== preview.contextDigest) fail('CONTEXT.md가 preview 뒤 변경되었습니다.', 'preview-mismatch')
    if (contentDigest(await readFile(join(root, 'prompts', 'llm-wiki.md'), 'utf8')) !== preview.principlesDigest) fail('LLM Wiki 운영 원칙이 preview 뒤 변경되었습니다.', 'preview-mismatch')
    if (preview.existingClaudeDigest && (!(await exists(join(target, 'CLAUDE.md'))) || contentDigest(await readFile(join(target, 'CLAUDE.md'), 'utf8')) !== preview.existingClaudeDigest)) fail('CLAUDE.md migration 원본이 preview 뒤 변경되었습니다.', 'preview-mismatch')
...
export async function advancePersonalWikiInit(state, input = {}, options = {}) {
  ...
  if (state.phase === 'clarification') {
    ...
    const combined = `${state.context}\n## Wiki initialization clarification\n- ${clean(input.answer, 400)}`; const proposed = offlinePlan(state.profile, combined, 'CONTEXT')
    ...
    const candidate = renderedFiles(state.memberId, proposed.plan, state.existingClaude); const existing = await migration(options.repoRoot, state.memberId, Object.entries(candidate).map(([path, content]) => ({ path, content })), { rootClaude: state.existingClaude }); const next = { version: 1, phase: 'preview', memberId: state.memberId, context: state.context, existingClaude: state.existingClaude, profileDigest: state.profileDigest, contextDigest: state.contextDigest, principlesDigest: state.principlesDigest, existingClaudeDigest: state.existingClaudeDigest, plan: proposed.plan, mode: 'offline-conservative', providerStatus: 'not-consented', migration: existing }; const preview = previewPersonalWikiInit(next)
    return { state: next, events: [event('map.proposed', { map: mapSummary(next.plan), mode: next.mode }), event('files.planned', { files: preview.files.map(file => file.path), requirements: preview.requirements, digest: preview.digest }), event('approval.required', { digest: preview.digest })], result: { preview, migration: existing } }
  }
  if (state.phase !== 'preview') fail('이미 완료된 세션입니다.', 'session-complete')
  const preview = previewPersonalWikiInit(state)
  if (input.action === 'preview') return { state, events: [event('files.planned', { files: preview.files.map(file => file.path), requirements: preview.requirements, digest: preview.digest }), event('approval.required', { digest: preview.digest })], result: { preview } }
...
export async function savePersonalWikiInit(state, input = {}, options = {}) {
  ...
  const preview = previewPersonalWikiInit({ ...state, phase: 'preview' }); await atomicSave(preview, options.repoRoot); return { state: { ...state, phase: 'saved' }, events: [event('files.written', { files: preview.files.map(file => file.path), requirements: preview.requirements }), event('session.completed', { phase: 'saved' })], result: { files: preview.files.map(file => file.path), requirements: preview.requirements } }
}
