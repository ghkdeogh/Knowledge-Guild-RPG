...
export function previewPersonalWikiInit(state) { if (!state || state.version !== 1 || state.phase !== 'preview') fail('초기화 preview 세션이 필요합니다.', 'invalid-session'); const files = renderedFiles(state.memberId, state.plan, state.rootClaude, state.compatibilityBootstrap); const keep = new Set((state.migration?.actions || []).filter(item => item.action === 'keep').map(item => `members/${state.memberId}/${item.path}`)); for (const path of keep) delete files[path]; const requirements = requiredOutcomes(state.memberId); return { kind: 'personal-wiki-init', mode: state.mode, migration: state.migration, profileDigest: state.profileDigest, primaryDigest: state.primaryDigest, primaryKind: state.primaryKind, legacyContextDigest: state.legacyContextDigest, requirements, plan: state.plan, files: Object.entries(files).map(([path, content]) => ({ path, content })), digest: digest({ files, requirements, mode: state.mode, migration: state.migration, profileDigest: state.profileDigest, primaryDigest: state.primaryDigest, primaryKind: state.primaryKind, legacyContextDigest: state.legacyContextDigest }) } }
