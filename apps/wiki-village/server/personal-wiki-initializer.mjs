...
async function readInputs(repoRoot, memberId) {
  ...
  return { scope, profile, context, profileDigest: contentDigest(profile) }
}
...
function validateProviderPlan(value, validEvidence, source) {
  ...
  for (const layer of ['raw', 'wiki', 'output']) {
    ...
    result[layer] = value[layer].map(candidate => {
      const id = safeId(candidate.id); const evidence = (Array.isArray(candidate.evidence) ? candidate.evidence : []).map(item => clean(item, 80)).filter(item => validEvidence.has(item))
      const candidateLabel = clean(candidate.label, 60); const purpose = clean(candidate.purpose, 120)
      if (!candidateLabel || !purpose || !evidence.length || (candidateLabel.length >= 20 && source.includes(candidateLabel)) || (purpose.length >= 20 && source.includes(purpose))) fail('제공자 근거가 유효하지 않습니다.', 'malformed-response')
...
async function proposedPlan(profile, context, options) {
  ...
  try {
    ...
    const refs = evidenceRefs(`${profile}\n${context}`); const response = await client.responses.create({ model: options.providerConfig?.model || 'gpt-5.6-terra', store: false, ...(options.providerConfig?.reasoningEffort ? { reasoning: { effort: options.providerConfig.reasoningEffort } } : {}), instructions: 'Return only JSON. Design a conservative personal LLM Wiki map. Use lowercase ASCII directory ids. Do not quote, repeat, or expose profile sentences. Evidence values must be exact allowed section references supplied below. Include only areas supported by the supplied profile/context, with at least one raw, wiki, and output item; otherwise return empty arrays.', input: `Allowed evidence references: ${refs.join(', ')}\nApproved PROFILE and CONTEXT (consent obtained):\n${profile}\n\n${context}`, text: { format: { type: 'json_schema', name: 'personal_wiki_map', strict: true, schema: providerSchema } } })
    const plan = validateProviderPlan(JSON.parse(response.output_text || '{}'), new Set(refs), `${profile}\n${context}`); return { plan, clarification: null, mode: 'llm-suggestion', providerStatus: 'used' }
...
function renderedFiles(memberId, plan, contextOriginal) {
  ...
  files[`${base}/CONTEXT.md`] = `${contextOriginal}${contextOriginal.endsWith('\n') ? '\n' : '\n\n'}${operationsBlock(plan)}`
...
async function migration(repoRoot, memberId, plan = null) {
  ...
  if (!structural.length && !context.includes(managedStart)) return { required: false, changes: [] }
  const changes = structural.map(path => ({ action: path === 'WIKI_SCHEMA.md' ? 'replace' : 'keep', path }))
  if (context.includes(managedStart)) changes.push({ action: 'replace', path: 'CONTEXT.md#Wiki 운영 규칙' })
  if (plan) for (const path of ['WIKI_SCHEMA.md', 'raw/CONTEXT.md', 'wiki/CONTEXT.md', 'wiki/index.md', 'wiki/log.md', 'output/CONTEXT.md']) if (!(await exists(join(target, path)))) changes.push({ action: 'add', path })
  return { required: true, changes }
}
export async function startPersonalWikiInit(input = {}, options = {}) {
  const memberId = safeMemberId(input.memberId); const { profile, context, profileDigest } = await readInputs(options.repoRoot, memberId); const proposed = await proposedPlan(profile, context, { ...options, providerApproved: input.providerApproved === true })
  if (!proposed.plan) { const state = { version: 1, phase: 'clarification', memberId, profile, context, profileDigest, providerApproved: input.providerApproved === true, clarificationCount: 0 }; return { state, events: [event('session.started', { command: 'personal-wiki-init', memberId }), event('provider.consent', { enabled: input.providerApproved === true }), event('clarification.required', { question: proposed.clarification, count: 1, maximum: 1 }), event('session.completed', { phase: 'insufficient-context' })], result: { status: 'insufficient-context', mode: proposed.mode, providerStatus: proposed.providerStatus } }
  }
  const existing = await migration(options.repoRoot, memberId, proposed.plan); const state = { version: 1, phase: 'preview', memberId, context, profileDigest, plan: proposed.plan, mode: proposed.mode, providerStatus: proposed.providerStatus, migration: existing }; const preview = previewPersonalWikiInit(state)
  return { state, events: [event('session.started', { command: 'personal-wiki-init', memberId }), event('provider.consent', { enabled: input.providerApproved === true }), event('map.proposed', { map: mapSummary(proposed.plan), mode: proposed.mode }), ...(existing.required ? [event('migration.required', { changes: existing.changes })] : []), event('files.planned', { files: preview.files.map(file => file.path), digest: preview.digest }), event('approval.required', { digest: preview.digest, reason: existing.required ? '기존 구조가 있어 migration 승인도 필요합니다.' : '미리보기와 digest를 확인한 뒤 명시적으로 승인해야 저장할 수 있습니다.' })], result: { preview, migration: existing, mode: proposed.mode, providerStatus: proposed.providerStatus } }
}
...
export function previewPersonalWikiInit(state) { if (!state || state.version !== 1 || state.phase !== 'preview') fail('초기화 preview 세션이 필요합니다.', 'invalid-session'); const files = renderedFiles(state.memberId, state.plan, state.context); return { kind: 'personal-wiki-init', mode: state.mode, migration: state.migration, profileDigest: state.profileDigest, plan: state.plan, files: Object.entries(files).map(([path, content]) => ({ path, content })), digest: digest({ files, mode: state.mode, migration: state.migration, profileDigest: state.profileDigest }) } }
async function atomicSave(preview, repoRoot) {
  const memberId = preview.files[0]?.path.match(/^members\/([^/]+)\//)?.[1]; const { root, target } = await safeExistingMember(repoRoot, memberId); const stage = resolve(root, `.personal-wiki-init-${randomUUID()}`); const backup = resolve(root, `.personal-wiki-context-${randomUUID()}`); const created = []; const createdDirs = []; let contextMoved = false
  const ensureTargetDirectory = async path => {
    const missing = []; let current = path
    while (current !== target && !(await exists(current))) { missing.push(current); current = dirname(current) }
    if (current !== target && current !== dirname(target)) { const stat = await lstat(current); if (stat.isSymbolicLink() || !stat.isDirectory()) fail('안전하지 않은 대상 디렉터리입니다.', 'unsafe-path') }
    for (const directory of missing.reverse()) { await mkdir(directory); createdDirs.push(directory) }
  }
  try {
    ...
    for (const file of preview.files) {
      ...
      if (file.path === `members/${memberId}/CONTEXT.md`) {
        if (!(await exists(destination)) || !(await lstat(destination)).isFile()) fail('기존 CONTEXT.md가 필요합니다.', 'collision', 409)
        const original = await readFile(destination, 'utf8')
        if (original.includes(managedStart) || file.content !== `${original}${original.endsWith('\n') ? '\n' : '\n\n'}${operationsBlock(preview.plan || { raw: [], wiki: [], output: [] })}`) fail('CONTEXT.md 변경 전제가 일치하지 않습니다.', 'preview-mismatch')
      } else if (await exists(destination)) fail('기존 파일을 덮어쓰지 않습니다.', 'collision', 409)
    }
    ...
    const contextPath = `members/${memberId}/CONTEXT.md`
    for (const file of preview.files.filter(file => file.path !== contextPath)) { const from = resolve(stage, file.path); const to = resolve(root, file.path); await ensureTargetDirectory(dirname(to)); await rename(from, to); created.push(to) }
    const stagedContext = resolve(stage, contextPath); const targetContext = resolve(root, contextPath)
    await rename(targetContext, backup); contextMoved = true
    await rename(stagedContext, targetContext); created.push(targetContext)
    await rm(backup, { force: true }); contextMoved = false
  } catch (error) {
    await Promise.all(created.map(file => rm(file, { force: true })))
    if (contextMoved && await exists(backup)) await rename(backup, resolve(root, `members/${memberId}/CONTEXT.md`)).catch(() => {})
    for (const directory of createdDirs.reverse()) await rmdir(directory).catch(() => {})
    throw error
  } finally { await rm(stage, { recursive: true, force: true }).catch(() => {}); await rm(backup, { force: true }).catch(() => {}) }
}
export async function advancePersonalWikiInit(state, input = {}, options = {}) {
  ...
  if (state.phase === 'clarification') {
    ...
    const existing = await migration(options.repoRoot, state.memberId, proposed.plan); const next = { version: 1, phase: 'preview', memberId: state.memberId, context: state.context, profileDigest: state.profileDigest, plan: proposed.plan, mode: 'offline-conservative', providerStatus: 'not-consented', migration: existing }; const preview = previewPersonalWikiInit(next)
...
export async function savePersonalWikiInit(state, input = {}, options = {}) {
  if (!state || state.phase !== 'approved' || input.action !== 'save') fail('승인된 초기화 세션이 필요합니다.', 'approval-required'); if (input.expectedDigest !== state.approvedDigest) fail('승인한 digest와 저장 요청이 일치하지 않습니다.', 'preview-mismatch'); if (state.migration.required) fail('기존 구조 migration은 자동 저장하지 않습니다. diff를 검토해 수동으로 적용해 주세요.', 'migration-manual-required')
