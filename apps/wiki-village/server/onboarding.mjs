...
export function onboardingMiddleware(options = {}) { return async (req, res, next) => { const pathname = String(req.url || '').split('?')[0]; if (pathname !== '/api/onboarding' && pathname !== '/api/onboarding-state') return next(); try { if (req.method === 'GET' && pathname === '/api/onboarding-state') return send(res, 200, { ...await getOnboardingState(options), capability: 'read-only' }); if (req.method !== 'POST') return send(res, 405, { error: '허용되지 않은 요청입니다.' }); const body = await bodyFrom(req); const { runWikiArchitect } = await import('../core/wiki-architect.mjs'); const commands = { analyze: 'analyze', 'preview-workspace': 'preview-workspace' }; const command = commands[body.action]; if (!command) throw Object.assign(new Error('개인 기본 초기화는 personal-wiki-init-cli를 사용해 주세요.'), { status: 400, code: body.action === 'preview' ? 'personal-wiki-init-required' : 'invalid-action' }); const response = await runWikiArchitect(command, body, options); return send(res, 200, { ...response.result, capability: 'read-only', events: response.events }) } catch (error) { const messages = { collision: '기존 member root 또는 workspace project가 있어 덮어쓰지 않았습니다.', 'invalid-member-id': 'member-id는 영문 소문자, 숫자, 하이픈만 사용할 수 있습니다.', 'invalid-input': '입력 또는 편집한 초안을 확인해 주세요.', 'invalid-blueprint': 'blueprint에 하나 이상의 안전한 page type이 필요합니다.', 'unsafe-path': '허용되지 않은 경로나 page type은 저장할 수 없습니다.', 'preview-mismatch': '미리보기 확인값이 일치하지 않습니다.', 'personal-wiki-init-required': '개인 Wiki 기본 초기화는 personal-wiki-init-cli를 사용해 주세요.' }; return send(res, error.status || 500, { error: messages[error.code] || '온보딩 요청을 완료하지 못했습니다.', code: error.code || 'onboarding-error', events: error.events || [] }) } } }
