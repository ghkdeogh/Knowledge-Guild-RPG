...
export function onboardingMiddleware(options = {}) { return async (req, res, next) => { const pathname = String(req.url || '').split('?')[0]; if (pathname !== '/api/onboarding' && pathname !== '/api/onboarding-state') return next(); try { const address = req.socket?.remoteAddress || ''; const remote = !loopback(address); if (req.method === 'GET' && pathname === '/api/onboarding-state') return send(res, 200, { ...await getOnboardingState(options), capability: remote ? 'read-only' : 'local-write' }); if (req.method !== 'POST') return send(res, 405, { error: '허용되지 않은 요청입니다.' }); const body = await bodyFrom(req); const { runWikiArchitect } = await import('../core/wiki-architect.mjs'); if (body.action === 'analyze') { const response = await runWikiArchitect('analyze', body, options); return send(res, 200, { ...response.result, capability: remote ? 'read-only' : 'local-write', events: response.events }) } if (body.action === 'preview-blueprint') { const response = await runWikiArchitect('preview', body, options); return send(res, 200, { ...response.result, capability: remote ? 'read-only' : 'local-write', events: response.events }) } if (readonly(options.readOnly)) throw Object.assign(new Error('Deployment storage is read-only'), { status: 403, code: 'read-only' }); if (remote) throw Object.assign(new Error('Local-only persistence'), { status: 403, code: 'local-only' }); if (body.action !== 'save-blueprint') return send(res, 400, { error: '알 수 없는 온보딩 작업입니다.', code: 'invalid-action' }); const response = await runWikiArchitect('save', body, options); return send(res, 200, { ...response.result, capability: 'local-write', events: response.events }) } catch (error) { const messages = { 'read-only': '이 배포 화면은 읽기 전용입니다. 내보내기 미리보기는 가능하지만 이 환경에서는 저장할 수 없습니다. 로컬 실행에서 저장할 수 있습니다.', 'local-only': 'LAN에서는 분석과 미리보기만 가능합니다. 저장은 이 컴퓨터의 localhost 또는 CLI에서 실행하세요.', collision: '기존 canonical project 또는 member root가 있어 덮어쓰지 않았습니다.', 'invalid-member-id': 'member-id는 영문 소문자, 숫자, 하이픈만 사용할 수 있습니다.', 'invalid-input': '입력 또는 편집한 초안을 확인해 주세요.', 'invalid-blueprint': 'blueprint에 하나 이상의 안전한 page type이 필요합니다.', 'unsafe-path': '허용되지 않은 경로나 page type은 저장할 수 없습니다.', 'preview-mismatch': '미리보기 확인값이 일치하지 않습니다. 다시 확인하세요.' }; return send(res, error.status || 500, { error: messages[error.code] || '온보딩 저장을 완료하지 못했습니다.', code: error.code || 'onboarding-error', events: error.events || [] }) } } }
