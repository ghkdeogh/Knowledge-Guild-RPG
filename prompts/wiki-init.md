...
2. 기존 member root `CLAUDE.md`가 있으면 byte를 보존하고, marker로 구분된 managed Wiki 운영 규칙 block만 승인된 migration에서 append 또는 replace한다. 없으면 승인된 `PROFILE.md` + `CONTEXT.md`로 개인화된 새 root `CLAUDE.md` 운영 계약을 만든다.
...
2. **오직** `members/{member-id}/PROFILE.md`, `CONTEXT.md`, 그리고 repo의 `prompts/llm-wiki.md` 원칙을 primary input으로 읽는다. `projects/`, 다른 member, raw 원본은 읽지 않는다. existing root `CLAUDE.md`가 있으면 planning input으로 조용히 쓰지 않고 migration 대상(보존·replace/managed update·backup)으로만 읽는다.
...
5. `PROFILE.md + CLAUDE.md`만 있고 `CONTEXT.md`가 없는 이전 전환 상태는 새 정상으로 취급하지 않는다. CLAUDE를 자동 추출하거나 입력으로 사용하지 않고, `context-missing` 안내와 온보딩 보완 경로만 제공하며 아무 파일도 쓰지 않는다.

## Preview before any write

근거마다 PROFILE 또는 CONTEXT의 **section reference**를 붙여, 원문 문장을 불필요하게 재노출하지 않고 다음을 제안한다. preview digest에는 PROFILE, CONTEXT, `prompts/llm-wiki.md` 세 입력의 hash를 모두 묶는다.
...
사용자는 digest를 명시적으로 승인해야 한다. existing root CLAUDE의 Wiki marker 또는 이전 Wiki 구조가 있으면 keep/add/replace migration diff를 먼저 제안한다. existing CLAUDE는 별도 migration 승인 뒤에만 managed block을 교체하며 원본은 `.wiki-migration-backup/<digest>/`에 복구 가능하게 보관한다. CLAUDE가 없는 새 member에는 CONTEXT의 개인 맥락을 바탕으로 root CLAUDE 운영 계약을 add한다. PROFILE과 CONTEXT는 절대 수정하지 않는다. 기존 원본, 다른 wiki 페이지, `WIKI_INDEX.md`, `ACTIVITY_LOG.md`, harness를 자동 삭제·이동·덮어쓰지 않으며, 이 legacy 항목은 unmanaged로 표시한다.
...
├─ CONTEXT.md                         # unchanged concise personal context
├─ CLAUDE.md                          # generated cross-agent contract + managed Wiki operations
...
`PROFILE.md` and `CONTEXT.md` are the approved personal inputs and are preserved unchanged. Saving is atomic: validate every path, reserved name, collision, and symlink boundary before writing; on failure leave no partial files. Root/layer `CLAUDE.md` files are cross-agent operational contracts, not public Wiki knowledge.
